...
import { Text, View, TouchableOpacity, Platform, NativeModules } from 'react-native'
...
export default class extends React.Component {
  ...
  render () {
    const {adaptiveStatusBar} = this.state
    return (
      <View>
        <View style={{height: adaptiveStatusBar && this.props.statusbarPadding ? STATUS_BAR_HEIGHT : 0, backgroundColor: 'white'}}/>
        <View style={
          [styles.navbar,
            {
        ...
        onLayout={() => StatusBarManager.getHeight(result => this.setState({adaptiveStatusBar: result.height > 0}))}
        >
          {
